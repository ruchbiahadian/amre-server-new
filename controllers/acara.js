...
export const updateAcara = (req, res) => {
  ...
  jwt.verify(token, "secretkey", (err, userInfo) => {
    ...
    const { namaAcara, reimbursement_status, plafon, absensi_status, maxAbsen, id, plafonLama } = req.body;
...
export const nonActiveAcaraAbsence = (req, res) =>{
  const token = req.cookies.accessToken;
  if(!token) return res.status(401).json("Not logged in!")
      ...
      jwt.verify(token, "secretkey", (err, userInfo)=>{
          ...
          const q = `SELECT * FROM acara WHERE absensi_status = "Nonaktif";`;
